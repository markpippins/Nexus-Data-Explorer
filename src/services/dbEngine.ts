...
export class DBEngine {
  ...
  // Whole-schema DDL: CREATE SCHEMA + CREATE TABLE for every table +
  // CREATE OR REPLACE VIEW for every view carried by discovery. Falls back
  // to a comment when discovery returned no objects for the schema.
  public static generateSchemaDDL(schema: SchemaObject, databaseName?: string): string {
    const target = databaseName ? `${databaseName}.${schema.name}` : schema.name;
    const lines: string[] = [
      `-- Schema DDL for ${target}`, `-- Generated by Data Explorer`, ``,
      `CREATE SCHEMA IF NOT EXISTS ${schema.name};`, ``,
    ];
    let count = 0;
    for (const table of schema.tables || []) {
      lines.push(this.generateDDL('CREATE_TABLE', schema.name, table), ``);
      count++;
    }
    for (const view of schema.views || []) {
      const definition = view.definition?.trim() || `-- (view definition was not returned by discovery)`;
      lines.push(`CREATE OR REPLACE VIEW ${schema.name}.${view.name} AS`, definition.endsWith(';') ? definition : `${definition};`, ``);
      count++;
    }
    for (const trigger of schema.triggers || []) {
      const definition = trigger.definition?.trim();
      if (definition) { lines.push(definition.endsWith(';') ? definition : `${definition};`, ``); count++; }
    }
    for (const procedure of schema.procedures || []) {
      const definition = procedure.definition?.trim();
      if (definition) { lines.push(definition.endsWith(';') ? definition : `${definition};`, ``); count++; }
    }
    if (count === 0) lines.push(`-- No tables/views were returned by discovery for ${schema.name}.`);
    return lines.join('\n');
  }
